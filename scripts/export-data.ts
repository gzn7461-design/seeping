...
async function main() {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.error("❌ COZE_SUPABASE_URL 和 COZE_SUPABASE_SERVICE_ROLE_KEY 环境变量必须设置");
    process.exit(1);
  }
  ...
  const exportDir = "/workspace/projects/export-data";
  ...
  for (const table of tables) {
    ...
    if (error) {
      console.error(`❌ ${table} 导出失败:`, error.message);
      continue;
    }

    const filePath = path.join(exportDir, `${table}.json`);
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf-8");
    console.log(`✅ ${table} 导出完成，共 ${data.length} 条记录 -> ${filePath}`);
  }

  // 导出建表 SQL
  console.log("\n📄 正在写入建表 SQL...");
  const sqlContent = `-- CommentHub 数据库建表 DDL
...
`;
  fs.writeFileSync(path.join(exportDir, "schema.sql"), sqlContent, "utf-8");
  console.log("✅ 建表 SQL 已导出 -> schema.sql");

  console.log("\n🎉 全部导出完成！文件位于 export-data/ 目录");
}
